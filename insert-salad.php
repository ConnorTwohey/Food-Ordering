<?php
  ...
  function insert_salad($name, $descrip, $price, $image, $stock, $calories, $fats, $size, $dressing) {
    try{
	...
	try{
		...
		$stmt = $pdo->prepare("INSERT INTO `Salad` (`Pid`, `Size`, `HasDressing`) VALUES (:pid, :size, :dressing);");
		$stmt->bindParam(':pid', $pid, PDO::PARAM_INT);
		$stmt->bindParam(':size', $size);
		$stmt->bindParam(':dressing', $dressing, PDO::PARAM_INT);
		$stmt->execute();
