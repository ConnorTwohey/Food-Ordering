...
<?php
		...
		$pnameErr = $descripErr = $priceErr = $imageErr = $stockErr = $calErr = $fatsErr = $iscoldErr = "";
        $pname = $descrip = $image = "";
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST"){
			...
			if (empty($image)) {
			  ...
			  if ($iscold == "" || $iscold < 0 || $iscold > 1) {
				$iscoldErr = "Is Cold is required";
			  }
			  
			  if($pnameErr == "" && $descripErr == "" && $priceErr == "" && $imageErr == "" && $stockErr == "" && $calErr == "" && $fatsErr == "" && $iscoldErr == ""){
					echo "Form has been sent.<br>";
					insert_dessert($pname, $descrip, $price, $image, $stock, $cal, $fats, $iscold);
